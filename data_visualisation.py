...
print(countrys_name)
gdp_cols = [col for col in gdp.columns if 'GDP' in col and col != 'COUNTRY']# leave only columns with "GDP"
Albania = row[gdp_cols]
Albania.index = [col.replace(' GPD', '')for col in gdp_cols] #Remove "GDP" becouse we must have only numbers without any letters
